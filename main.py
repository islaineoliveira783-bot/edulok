...
def caminho(arquivo):
    return str(BASE_DIR / arquivo)

def abrir_jogo(arquivo):
    subprocess.Popen([sys.executable, caminho(arquivo)], cwd=BASE_DIR)
...
img_fundo = pygame.image.load(caminho("assets/images/fundodatela.jpeg")).convert()
img_fundo = pygame.transform.scale(img_fundo, (LARGURA, ALTURA))

img_titulo = pygame.image.load(caminho("assets/images/titulo.1.png")).convert_alpha()
largura_logo = 230
...
sprites_robo = [
    pygame.image.load(caminho("assets/images/FRONT.1.PNG")).convert_alpha(),
    pygame.image.load(caminho("assets/images/ESQUERDA.1.PNG")).convert_alpha(),
    pygame.image.load(caminho("assets/images/ATRÁS.1.PNG")).convert_alpha(),
    pygame.image.load(caminho("assets/images/DIREITA.1.PNG")).convert_alpha()
]
...
def desenhar_perfil(mouse):
    ...
    pygame.draw.rect(fundo, (255,255,255,35) if hover else (255,255,255,18), fundo.get_rect(), border_radius=20)
...
def desenhar_botao(x, y, texto, mouse, superficie_destino=tela):
    ...
    pygame.draw.rect(fundo, (255,255,255,45) if hover else (255,255,255,20), fundo.get_rect(), border_radius=15)
...
def desenhar_card(x, y, titulo, subtitulo, mouse, imagem_nome):
    largura = 230
    altura = 300
    ...
    pygame.draw.rect(brilho, (80,120,255,70) if hover else (80,120,255,35), brilho.get_rect(), border_radius=30)
    ...
    try:
        img_card = pygame.image.load(caminho(f"assets/images/{imagem_nome}")).convert_alpha()
        img_card = pygame.transform.smoothscale(img_card, (80, 80))
        tela.blit(img_card, img_card.get_rect(center=(x + 115, y + 140)))
    except:
...
def desenhar_nivel(x, y, numero, mouse):
    ...
    pygame.draw.rect(fundo, (255,255,255,40) if hover else (8,18,55,215), fundo.get_rect(), border_radius=22)
    tela.blit(fundo, (x, y))
...
def tela_menu(mouse, tempo):
    ...
    btn_port = desenhar_card(390, 260, "CÓDICES LETRAIS", "Leitura e interpretação", mouse, "chave.png")
    btn_mat = desenhar_card(660, 260, "SUSSURROS NUMÉRICOS", "Desafios e lógica", mouse, "chave.png")
    btn_aux = desenhar_card(930, 260, "AUXILIADOR", "Peça ajuda", mouse, "chave.png")
...
while True:
    ...
    if estado == MENU:
        ...
        for evento in eventos:
            if evento.type == pygame.MOUSEBUTTONDOWN and evento.button == 1:
                if btn_port.collidepoint(mouse):
                    estado = PORTUGUES

                elif btn_mat.collidepoint(mouse):
                    estado = MATEMATICA
        ...
        for evento in eventos:
            if evento.type == pygame.MOUSEBUTTONDOWN and evento.button == 1:
                if voltar.collidepoint(mouse):
                    estado = MENU

                for numero, rect in niveis:
                    if rect.collidepoint(mouse):
                        if numero == 1:
                            abrir_jogo("games/robo_detector.py")
                        else:
                            print(f"Fase {numero} ainda não criada.")
        ...
        for evento in eventos:
            if evento.type == pygame.MOUSEBUTTONDOWN and evento.button == 1:
                ...
                for numero, rect in niveis:
                    if rect.collidepoint(mouse):
                        if numero == 1:
                            abrir_jogo("games/matematica.py")
                        else:
                            print(f"Fase {numero} ainda não criada.")
        ...
        for evento in eventos:
            if evento.type == pygame.MOUSEBUTTONDOWN and evento.button == 1:
                ...
                if serie1 and serie1.collidepoint(mouse):
                    serie_escolhida = "1º ANO"

                elif serie2 and serie2.collidepoint(mouse):
                    serie_escolhida = "2º ANO"
                    scroll_y = 0

                elif serie3 and serie3.collidepoint(mouse):
                    serie_escolhida = "3º ANO"
